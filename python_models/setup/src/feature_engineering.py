...
def extract_all_features(resume, skill_vocab, domain_requirements):
    ...
    engineer = WeightedFeatureEngineer(skill_weight=0.5, test_weight=0.5)
    credibility_score = engineer.compute_credibility_score(skill_match_ratio, test_score_norm)
    competency_score = engineer.compute_competency_score(skill_match_ratio, test_score_norm)
    potential_score = engineer.compute_potential_score(skill_match_ratio, test_score_norm, years_experience)
    consistency_score = engineer.compute_consistency_score(skill_match_ratio, test_score_norm)
...
class FeatureVectorBuilder:
    ...
    def build_numeric_branch(self, features):
        """Branch 2: Numeric features - NOW BALANCED WITH TEST SCORE PRIORITY"""
        scaled = features['scaled_numeric_features']
        return np.array([
            # PRIMARY SIGNALS (equal importance)
            features['test_score_norm'],           # [0] What they CAN do
            features['skill_match_ratio'],         # [1] What they CLAIM

            # COMPOSITE FEATURES (the intelligence!)
            features['credibility_score'],         # [2] Are they lying?
            features['competency_score'],          # [3] Overall 50/50 score
            features['potential_score'],           # [4] Growth potential
            features['consistency_score'],         # [5] Claims vs performance

            # SUPPORTING FEATURES
            scaled[2],                             # [6] project_count_scaled
            scaled[0],                             # [7] years_experience_scaled
        ])
    ...
    def get_feature_dimensions(self):
        ...
        numeric_dim = 8  # CHANGED FROM 4 TO 8!
...
# ============================================================================
# Weighted Feature Engineering (NEW - ADD THIS)
# ============================================================================

class WeightedFeatureEngineer:
    """Creates composite scores balancing skills and test performance"""
    
    def __init__(self, skill_weight=0.5, test_weight=0.5):
        self.skill_weight = skill_weight
        self.test_weight = test_weight
    
    def compute_credibility_score(self, skill_match_ratio, test_score_norm):
        """Credibility: How credible are their claimed skills?"""
        # Penalty if they claim high skills but low test score
        lying_penalty = 0
        if skill_match_ratio > test_score_norm:
            lying_penalty = (skill_match_ratio - test_score_norm) * 0.5
        
        base_credibility = min(skill_match_ratio, test_score_norm)
        credibility = max(0, base_credibility - lying_penalty)
        
        return credibility
    
    def compute_competency_score(self, skill_match_ratio, test_score_norm):
        """Competency: Overall demonstrated competency (50/50 balance)"""
        return (self.skill_weight * skill_match_ratio + 
                self.test_weight * test_score_norm)
    
    def compute_potential_score(self, skill_match_ratio, test_score_norm, years_experience):
        """Potential: Can they grow into the role?"""
        test_contribution = test_score_norm * 0.6
        
        # Bonus if test > skills (quick learner)
        learning_bonus = 0
        if test_score_norm > skill_match_ratio:
            learning_bonus = (test_score_norm - skill_match_ratio) * 0.3
        
        exp_norm = min(years_experience / 10.0, 1.0)
        exp_contribution = exp_norm * 0.1
        
        potential = min(1.0, test_contribution + learning_bonus + exp_contribution)
        return potential
    
    def compute_consistency_score(self, skill_match_ratio, test_score_norm):
        """Consistency: How well do claims match performance?"""
        gap = abs(skill_match_ratio - test_score_norm)
        consistency = 1.0 - gap
